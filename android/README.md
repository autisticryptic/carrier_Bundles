...
### 3. Xiaomi ROM 运营商配置与基带 inventory

Xiaomi 15 Ultra (`xuanyuan`) 现在有独立构建器：[`xiaomi`](./xiaomi/)。默认输入是完整 OTA mirror，也支持本地 fastboot `.tgz/.tar`。它从系统分区提取 `CarrierConfig`、APN/ePDG XML 并写入 `carrier_profiles`，同时记录 `NON-HLOS.bin`、`modem*.img`、`dsp*.img` 等基带相关制品的来源、路径、大小和哈希。firmware-only 包不能作为 profile 数据源，只能作为基带 inventory 辅助输入。
